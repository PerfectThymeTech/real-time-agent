resource "random_uuid" "uuid_webhook_token" {}
...
resource "azurerm_eventgrid_system_topic_event_subscription" "eventgrid_system_topic_event_subscription" {
  ...
  webhook_endpoint {
    url                               = "https://${azurerm_container_app.container_app_backend.ingress[0].fqdn}/v1/calls/incoming?token=${random_uuid.uuid_webhook_token.result}"
